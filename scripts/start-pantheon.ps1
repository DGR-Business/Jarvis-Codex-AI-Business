...
$runtimePath = Join-Path $tmpRoot "pantheon-server-$Port.json"
...
if (Test-Path -LiteralPath $credentialPath) {
  try {
    ...
    throw "Pantheon could not unlock its local OpenAI credential profile. Re-run Connect OpenAI for this Windows account."
  }
}
...
if (Test-Path -LiteralPath $runtimePath) {
  ...
  catch { throw "Pantheon found an unreadable runtime ownership file at $runtimePath." }
}

$health = Get-PantheonHealth
...
if ($health -and $metadata -and [string]$metadata.configFingerprint -ne $fingerprint) {
  ...
  & (Join-Path $PSScriptRoot "stop-pantheon.ps1") -Port $Port
...
if (-not $health) {
  if ($metadata) {
    ...
    if ($staleProcess) {
      throw "Pantheon found a stale ownership file for a still-running process. Stop it safely before restarting."
    }
  ...
  $startInfo.Arguments = "scripts/serve-pantheon.js $Port"
  ...
  foreach ($prefix in @("PANTHEON", "JARVIS")) {
    $startInfo.EnvironmentVariables["${prefix}_LIVE_MODE"] = "0"
    $startInfo.EnvironmentVariables["${prefix}_OPERATOR_BOOTSTRAP"] = $bootstrapToken
    $startInfo.EnvironmentVariables["${prefix}_CONTROL_TOKEN"] = $controlToken
    $startInfo.EnvironmentVariables["${prefix}_RUNTIME_INSTANCE_ID"] = $instanceId
  }
...
for ($attempt = 0; $attempt -lt 60 -and $health.ok -ne $true; $attempt += 1) {
  ...
  Start-Sleep -Milliseconds 500
  $refreshedHealth = Get-PantheonHealth
